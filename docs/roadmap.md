...
- [x] 2.1 Bevy App 配置 - 创建 Bevy 应用基础框架，配置必要插件
- [x] 2.2 透明无边框窗口 - 配置 `WindowPlugin` 实现透明无边框悬浮窗
- [x] 2.3 置顶显示 - 实现窗口 `AlwaysOnTop` 功能
- [x] 2.4 GLB 模型加载 - 实现 GLB/glTF 格式 3D 模型加载与渲染
- [x] 2.5 摄像机设置 - 配置适合桌面宠物显示的摄像机参数
- [x] 2.6 光照设置 - 配置合适的光照效果
- [x] 2.7 窗口拖拽 - 实现鼠标拖拽移动窗口功能
- [x] 2.8 点击交互 - 实现点击宠物触发事件
...
- [x] 3.1 脚本数据结构 - 定义 `BehaviorScript`, `Keyframe`, `Action` 数据结构
- [x] 3.2 脚本加载器 - 实现 RON 格式行为脚本的加载与解析
- [x] 3.3 Replay 插件 - 创建 Bevy Replay 插件，管理脚本回放状态
- [x] 3.4 关键帧系统 - 实现按时间轴执行关键帧动作的系统
- [x] 3.5 动画播放 - 实现 `PlayAnimation` 动作，播放模型内置动画
- [x] 3.6 移动动作 - 实现 `MoveTo` 动作，控制宠物位置移动
- [x] 3.7 缩放动作 - 实现 `Scale` 动作，控制宠物缩放
- [x] 3.8 随机选择 - 实现 `Random` 动作，随机选择下一个脚本
- [x] 3.9 脚本切换 - 实现脚本结束后自动切换到 `next` 脚本
- [x] 3.10 内置脚本 - 创建 `idle`, `walk`, `happy`, `sleep` 内置脚本
- [x] 3.11 交互触发 - 实现交互事件（点击）触发脚本切换
- [x] 3.12 弹跳动作 - 实现 `Bounce` 动作及对应行为脚本
- [x] 3.13 旋转动作 - 实现 `Spin` 动作，控制宠物旋转行为
