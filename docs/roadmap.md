...
- [ ] 1.1 项目初始化 - 创建 Cargo workspace，配置 `Cargo.toml`
- [ ] 1.2 创建 crate 结构 - 创建 `app`, `common`, `tray`, `theater`, `manager` 五个 crate
- [ ] 1.3 共享库 `common` - 实现数据模型、配置读写、存储服务
- [ ] 1.4 配置存储系统 - 使用 config crate 实现配置文件读写
- [ ] 1.5 主进程框架 - 实现进程管理器，支持启动/停止/监控子进程
- [ ] 1.6 进程间通信 - 实现基于配置文件的进程间状态同步机制
- [ ] 1.7 系统托盘进程 - 使用 `tray-icon` 实现托盘图标和菜单
